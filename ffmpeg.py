...
class FFmpegReader():
    ...
    def __init__(self, filename, inputdict=None, outputdict=None, verbosity=0):
        ...
        if ("-vframes" in outputdict):
            ...
            self.inputfps = int(outputdict["-r"])
            self.inputduration = float(viddict["@duration"])
            self.inputframenum = int(round(self.inputfps*self.inputduration) + 1)
        ...
        if self.inputframenum == -1:
            ...
            self.inputframenum = int(check_output(probecmd).decode().split('\n')[0])
